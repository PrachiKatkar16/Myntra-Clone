...
function Women({ addToWishlist }) {
  ...
  useEffect(() => {
    fetch('https://myntra-clone-2-jz9i.onrender.com/cloth/category/Womens')
      .then(response => {
        if (!response.ok) {
    ...
    <Box>
      ...
      <Box padding="4">
        <Grid templateColumns="1fr 3fr" gap={6}>
          <Box>
            <Text fontWeight="bold" fontSize='4xl' mb={4} color='#ee64a6'>Womens: {filteredProducts.length}</Text>
          ...
          <Box>
            ...
            <Grid templateColumns="repeat(4, 1fr)" gap={6}>
              {currentProducts.map(product => (
                <GridItem key={product._id} className="product-card" p={2} boxShadow="md" borderRadius="md">
                  <VStack spacing={2} align="start">
                    <Image src={product.images} alt={product.title} className="product-image" objectFit="cover" />
